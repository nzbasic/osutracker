...
export default function UserPlays({ plays, currentTop, country }) {
  ...
  useEffect(() => {
    if (plays.length != 0) {
      ...
      for (let i=plays.length-1; i>=0; i--) {
        ...
        lastPlays.sort((play1, play2) => parseInt(play2.pp) - parseInt(play1.pp))
  ...
  const Play = ({ data, index }) => (
    ...
    <div className={`${data.added ? "bg-green-500 text-main-one" : data.removed ? "bg-red-600 text-main-one" : "bg-main-one"} flex rounded-md shadow-md p-2 my-2 justify-between w-full text-xs md:text-sm lg:text-base`}>
      <div className="flex">
  ...
  return (
    <div className="w-full max-w-full flex flex-col">
      <div className="self-center p-2 mb-4 bg-main-one rounded-md shadow-md flex">
        <div className={`${currentIndex > 0 ? "block" : "invisible"} mr-2 hover:text-main-four font-extrabold cursor-pointer`} onClick={() => setCurrentIndex(currentIndex-1)}>⟵</div>
        <div className="font-semibold">{new Date(playsHistory[currentIndex].date).toDateString()}</div>
        <div className={`${currentIndex < plays.length ? "block" : "invisible"} ml-2 hover:text-main-four font-extrabold cursor-pointer`} onClick={() => setCurrentIndex(currentIndex+1)}>⟶</div>
      </div>
