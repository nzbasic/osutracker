...
export class User {
    ...
    @Prop(raw({ total: { type: Number }, available: { type: Number } }))
    kudosu: {
        total: number;
        available: number;
    };
    ...
    @Prop(raw([{ playmodes: { type: Array } }])) groups: Array<{
    ...
    @Prop(raw([{ start_date: { type: String }, count: { type: Number } }]))
    ...
    @Prop(
        raw([
            { achieved_at: { type: String }, achievement_id: { type: Number } },
        ])
    )
    ...
    @Prop(raw({ mode: { type: String }, data: [{ type: Number }] }))
    rank_history: {
        mode: string;
        data: Array<number>;
    };
