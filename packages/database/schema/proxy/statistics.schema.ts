...
export class Statistics {
  ...
  @Prop(raw({ current: { type: Number }, progress: { type: Number } })) level: {
    current: number;
    progress: number;
  };
  ...
  @Prop(raw({ ss: { type: Number }, ssh: { type : Number }, s: { type: Number }, sh: { type: Number }, a: { type: Number } })) grade_counts: {
    ss: number;
    ssh: number;
    s: number;
    sh: number;
